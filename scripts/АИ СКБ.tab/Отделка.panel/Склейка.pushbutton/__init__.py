    ...
    for element in elements:
        ...
        for i in geom:
            if isinstance(i, Solid):
                ...
                all_element_dict[element.Id.IntegerValue] += [j for j in list(i.GetInstanceGeometry()) if isinstance(j, Solid)]
        ...
        for room_solid in room_solids:
            for obj_key, obj_solids in objects.items():
                for obj_solid in obj_solids:
                    new_solid = BooleanOperationsUtils.ExecuteBooleanOperation(room_solid, obj_solid, BooleanOperationsType.Union)
                    is_intersect = round((new_solid.SurfaceArea - room_solid.SurfaceArea - obj_solid.SurfaceArea) + (new_solid.Volume - room_solid.Volume - obj_solid.Volume), 10) != 0
                    if is_intersect:
                        r_obj = doc.GetElement(ElementId(obj_key))
                        r_room = doc.GetElement(ElementId(room_key))
                        room_number = r_room.LookupParameter("ROM_Номер").AsString()
                        room_name = r_room.LookupParameter("Имя").AsString()
                        res_string = "{} {}".format(room_number, room_name)
                        r_obj.LookupParameter("BS_Текст").Set(res_string)
                        break
	else:
		r_obj = doc.GetElement(ElementId(obj_key))
		r_obj.LookupParameter("BS_Текст").Set("Не заполнено")
    t.Commit()          
